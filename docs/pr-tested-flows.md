...
curl -X POST http://localhost:3001/entity -H 'x-tenant-id: public' -H 'content-type: application/json' \
  -d '{"entity_id":"merchant_demo"}'
```
**Response — `HTTP 200`**
```json
{
  "entity_id": "merchant_demo",
  "created_at": "2026-06-24T19:27:37.552Z"
}
```
Calling again with the same `entity_id` returns `HTTP 200` with the identical record (no new row).
Once provisioned, the add-card / vault flows below find the existing record and skip the
deprecated lazy auto-create path (no `add_flow_auto_create` warning is logged).
Blank `entity_id` → `HTTP 400`; missing `entity_id` field → `HTTP 422`; missing `x-tenant-id` → `HTTP 400`.

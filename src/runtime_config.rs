...
//! dispatch, plus seeding and status. The one thing it never generates is validation:
//! [`RuntimeConfigEntry`] requires [`RuntimeConfigValidate`] as a supertrait, so a config
//! type must implement it by hand — there is no default and no macro syntax to opt out.
...
/// be registered in [`runtime_configs!`] without one.
...
///
/// The `KEY` binding is generated by [`runtime_configs!`] — never write one by hand, or
/// the key and the struct can drift apart again. [`RuntimeConfigValidate`] is *not*
/// generated: every config type implements it itself.
...
///
/// Grammar: `field: Type = "configs row key"`. `Type` must implement
/// [`RuntimeConfigValidate`] by hand — there is no default validator, so the
/// `RuntimeConfigEntry` impl this macro generates cannot compile without one.
///
/// Generates, per entry: a seed field on `RuntimeConfig::Enabled`, the
/// `RuntimeConfigEntry` impl (the `KEY` binding only — validation is never generated), a
/// [`RuntimeConfigUpdate`] variant tagged with the key, a [`RuntimeConfigSeeds`] field,
/// and the seed/dispatch arms. The update variant is named after the type, so the
/// declaration carries no redundant second name; the key stays a literal because
/// serde's `rename` cannot read an associated const.
macro_rules! runtime_configs {
    (
        $(
            $(#[$meta:meta])*
            $field:ident : $ty:ident = $key:literal
        ),* $(,)?
    ) => {
...
runtime_configs! {
    ...
    kv_config: KvRuntimeConfigValues = "kv_config",

    // Add a runtime config by adding one line here — the type must implement
    // `RuntimeConfigValidate` by hand (see the impl for `KvRuntimeConfigValues` below).
    // The seed field, the `RuntimeConfigEntry` impl, the update variant, the update
    // dispatch, seeding and status all follow from this declaration.
}
...
/// Required by `RuntimeConfigEntry`'s supertrait bound — not wired in through
/// `runtime_configs!`, which never generates a validator.

...
pub async fn middleware(
    ...
    let request_body = hyper::body::to_bytes(body)
        ...
        .change_context(error::ApiError::MiddlewareError(
            "Failed to read request body for jwe decryption".into(),
        ))
        .report_unwrap()?;

    let jwt = String::from_utf8(request_body.to_vec())
        .change_context(error::ApiError::MiddlewareError(
            "Could not convert to UTF-8".into(),
        ))
        .report_unwrap()?;

    let jwe_decrypted = decrypt_jwe(
        &jwt,
        &state.config.locker_key.locker_private_key,
        jwe::RSA_OAEP_256,
    )
    .change_context(error::ApiError::MiddlewareError(
        "Jwe decryption failed".into(),
    ))
    .report_unwrap()?;

    let jws_verified = verify_sign(jwe_decrypted, &state.config.locker_key.tenant_public_key)
        .change_context(error::ApiError::MiddlewareError(
            "Jws verification failed".into(),
        ))
        .report_unwrap()?;
    ...
    let response_body = hyper::body::to_bytes(body)
        .await
        .change_context(error::ApiError::MiddlewareError(
            "Failed to read response body for jws signing".into(),
        ))
        .report_unwrap()?;

    let jws_signed = jws_sign_payload(&response_body, &state.config.locker_key.locker_private_key)
        .change_context(error::ApiError::MiddlewareError(
            "Jws signing failed".into(),
        ))
        .report_unwrap()?;

    let jwe_encrypted = encrypt_jwe(
        jws_signed.as_bytes(),
        &state.config.locker_key.tenant_public_key,
    )
    .change_context(error::ApiError::MiddlewareError(
        "Jwe encryption failed".into(),
    ))
    .report_unwrap()?;
